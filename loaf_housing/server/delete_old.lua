        ...
        AddEventHandler("loaf_housing:updateJoined", function()
            ...
            if xPlayer and xPlayer.identifier then
                if not updatedPlayers[tostring(xPlayer.source)] then
                    ...
                    print(GetPlayerName(xPlayer.source) .. " tried to update their last login, probably using an executor.")
                end
            end
        end)
        ...
        while true do            
            ...
            MySQL.Async.fetchAll("SELECT `identifier` FROM `loaf_last_login` WHERE `login_time` <= @delete", {
                ...
                for k, v in pairs(data) do
                    ...
                    print(string.format("Removed %s from loaf_housing since they have been inactive for %s seconds.", v.identifier, Config.DeleteHouse))
                end
            end)
